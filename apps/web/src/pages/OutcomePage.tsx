...
function statusDescription(status: OutcomeView["status"]): string {
  switch (status) {
    ...
    case "confirmed":
      return "The provider handoff was confirmed. No completed provider result has been reported yet.";
    case "accepted":
      return "The provider accepted the attributed request.";
    case "completed":
      return "The provider reported the defined result as completed.";
    case "cancelled":
      return "The provider reported that the result was cancelled.";
    case "refunded":
      return "The provider reported a refund.";
    case "disputed":
      return "The provider result is disputed.";
  }
}
  ...
  if (loading) {
    return (
      <main id="main-content" className="secure-page">
        <p className="quiet-status" role="status" aria-live="polite">Loading task record…</p>
      </main>
    );
  }
    ...
    return (
      <main id="main-content" className="secure-page">
        <section className="success-receipt">
          <span className="workspace-label">Outcome unavailable</span>
          <h1>Task record could not be loaded</h1>
          <p>{error}</p>
          <Link className="button button--primary" to="/">New task</Link>
        </section>
      </main>
    );
  }
  ...
  return (
    <main id="main-content" className="secure-page secure-page--success">
      <section className="success-receipt outcome-receipt" aria-live="polite">
        <span className="workspace-label">Task record</span>
        ...
        <section className="outcome-events" aria-labelledby="outcome-events-title">
          <h2 id="outcome-events-title">Provider events</h2>
          {events.length === 0 ? (
        ...
        <div className="receipt-actions">
          ...
          <Button variant="secondary" onClick={() => void load()}>Refresh status</Button>
          <Link className="button button--quiet" to="/">New task</Link>
        </div>

        <p className="confirmation-note">
          The provider continuation link exists only in this confirmed browser session. A completed commercial result still requires an authenticated provider event.
        </p>
      </section>
    </main>
  );
